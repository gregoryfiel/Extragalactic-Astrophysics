...
def main():
    ...
    ex_gal = np.genfromtxt(cxt_file) 
    lam_ex = ex_gal[:, 0][ex_gal[:, 1] > 0]
    flux_ex = ex_gal[:, 1][ex_gal[:, 1] > 0]
    error_ex = ex_gal[:, 2][ex_gal[:, 1] > 0]

    analyzer = SpectralAnalysis(lam_ex, flux_ex, error_ex)
